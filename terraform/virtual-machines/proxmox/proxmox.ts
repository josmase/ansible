...
export interface VirtualMachine {
  ...
  on_boot: boolean;
  /** Explicit worker storage contract. Sizes use Proxmox's GiB syntax. */
...
function virtualMachineToVmQemuConfig(value: VirtualMachine): VmQemuConfig {
  const config = {
    ...
    lifecycle: {
      // Network remains externally managed; worker disks are now explicit so
      // 204/205/206 share one declarative storage contract.
      ignoreChanges: ["network"],
    },
    onboot: value.on_boot,
    ...(value.boot_disk_size && value.data_disk_size && value.disk_storage
      ? {
          disk: [
            { type: "scsi0", storage: value.disk_storage, size: value.boot_disk_size },
            { type: "scsi1", storage: value.disk_storage, size: value.data_disk_size },
          ],
        }
      : {}),
  };

  // The generated provider schema is regenerated by `cdktf get`; keeping the
  // cast here lets older checked-in provider bindings remain usable during the
  // transition while emitting the same Terraform disk blocks.
  return config as unknown as VmQemuConfig;
}
